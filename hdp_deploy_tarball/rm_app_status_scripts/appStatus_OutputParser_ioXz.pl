...
die "Input file not provided\n$0 <input_xz_file> [output_file_suffixd with xz]\n" unless($ip and -f $ip);
my $xzOut = undef;
if ($op){
    $xzOut = new IO::Compress::Xz $op;
    print STDERR "xz failed: $XzError\n" unless (defined($xzOut));
    ...
    if ( $vals[7] =~ /RUNNING/i){
        ...
        if (exists $time_user_queue{$ts}->{$vals[5]}{$vals[6]}){
            ...
            unless (exists $time_user_queue{$ts}->{$vals[5]}{$vals[6]}{$vals[4]}){
                ...
                #$napps += 1; $ncontaienrs += $vals[9]; $nvcores += $vals[12]; $mem += $vals[11]; $n_qusage += $vals[10], $n_ucsage += $vals[15]; $n_r_rqts += $vals[19];
            }
            #if ($ts == 28){
...
foreach my $ts(sort {$a <=> $b} keys %time_user_queue){
    ...
    foreach my $user (sort keys $time_user_queue{$ts}){
        foreach my $queue(sort keys $time_user_queue{$ts}{$user}){
            ...
            #"$ts $user $user %s %s %s %s ", $t->{'num_apps_running'}, $t->{'num_containers_running'}, $t->{'total_vcores'}, $t->{'total_memory'}; 
