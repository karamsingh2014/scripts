...
class HTTPSConnectionV3(httplib.HTTPSConnection):
    ...
    def connect(self):
        ...
        try:
            ...
            print >>sys.stderr, "INFO Trying SSLv2/3."
...
class YARN:
        ...
        for attempt in range(retry):
            ...
            try:
               ...
               if isinstance(e, urllib2.HTTPError):
                  ...
                  print >>sys.stderr, "INFO: Sleep 10 secs due to URL error before retry"
                  time.sleep(10)
    ...
    def query_yarn_web_service(cls, ws_url, user, query_headers=None, data=None, use_xml=False, http_method='GET', 
        ...
        print >>sys.stderr, "INFO: query_yarn_web_service start Accessing " + ws_url
    ...
    def get_ats_json_code_data_headers(cls, url_comp, user, return_exception_json=False, do_not_use_curl_in_secure_mode=False, use_user_auth_in_un_secure_mode=False, user_delegation_token=None,
       ...
       if url_comp is not None and len(url_comp.strip()) > 0:
          ws_url += '/' + url_comp
       print >>sys.stderr, "DEBUG: get_ats_json_code_data_headers ws_url = %s" % ws_url
    ...
    def get_ats_json_data(cls, url_comp, user, return_exception_json=False, do_not_use_curl_in_secure_mode=False, use_user_auth_in_un_secure_mode=False, user_delegation_token=None, 
        ...
        if ret_code < 100 or ret_code > 299:
           if ret_data is not None and 'exception' in ret_data:
               print >>sys.stderr, "ERROR: Got Exception in repsonse JSON %s by user %s for urlcomp %s" %(str(ret_data), user, url_comp)
    ...
    def access_ats_ws_path(cls, url_comp, ids, app_id, user, expected_values=1, use_user_auth_in_un_secure_mode=False, delegation_token=None, cookie=None):
        ...
        print >>sys.stderr, "INFO: list of entites " + str(len(p['entities']))
        if len(p['entities']) < expected_values:
           return
        for e in p['entities']:
            if e['entity'] in ids:
                print >>sys.stderr, "DEBUG: ATS WS " + url_comp + " contains " + e['entity'] + " which is also ran by our application " + app_id
    ...
    def __validate_json_related_identities__(cls, entity_type, eid, related_identities, part_uri):
        ...
        if related_identity in related_identities:
            for i in related_identities[related_identity]:
                print >>sys.stderr, "INFO: " + str(eid) + " contains " + str(i)
    ...
    def __validate_json_primary_filters__(cls, entity_type, app_id, eid, ids_dict, owner, primary_filters, part_uri, user,
        ...
        if entity_type in ['TEZ_DAG_ID', 'TEZ_APPLICATION_ATTEMPT']:
            ...
            if entity_type == 'TEZ_DAG_ID':
                ...
                print >>sys.stderr, "INFO: " + str(eid) + " having dagName " + str(dn)
        ...
        if entity_type in ['TEZ_VERTEX_ID', 'TEZ_TASK_ID', 'TEZ_TASK_ATTEMPT_ID']:
            ...
            for d in primary_filters['TEZ_DAG_ID']:
                print >>sys.stderr, "INFO: " + str(eid) + " dagId : " + str(d)
    ...
    def __validate_json_for_ids__(cls, entity_type, app_id, eid, ids_dict, owner, url_comp_to_query=None, fields_to_compare=None,
        ...
        if p is None:
            if assert_fail_on_error:
                ...
                print >>sys.stderr, 'ERROR: Got empty JSON %s for query with user=%s' % (str(p), user)
                return False
        if type(p) != dict:
            if assert_fail_on_error:
                assert type(p) == dict, 'For query with user=%s, Response is not in JSON: %s' % (user, str(p))
            else:
                print >>sys.stderr, 'ERROR: For query: %s with user=%s, Response is not in JSON: %s' % (str(entity_type + part_uri),user, str(p))
        ...
        if 'exception' in p or len_of_data <= 0:
            if assert_fail_on_error:
                ...
                print >>sys.stderr, 'ERROR: Got empty or exception JSON %s for query %s with user=%s' % (str(p), str(entity_type + part_uri),user)
                #traceback.print_stack()
                return False
        if entity_type_comp:
            '''
            ...
            print >>sys.stderr, "INFO: " + str(eid) + " entityType = " + str(e_type)
        ...
        if entity_comp:
            ...
            if again:
                ...
                ety.append(p['entity'])
            print >>sys.stderr, "INFO: " + str(eid) + " entity = " + str(ety)
            ...
            for ey in ety:
                if json_data is not None:
                    ...
                    if re.search(m, ey) is not None:
                        print >>sys.stderr, "INFO: Using regex search instead of pattern equal to " + m + " " + str(re.search(m, ey))
                        found = True
                        break
                if eid == ey:
                    found = True
        ...
        if start_time_comp:
            print >>sys.stderr, "INFO: " + str(eid) + " starttime = " + str(p['starttime'])
        ...
        if events_comp:
            ...
            if again:
                for ev in p['events']:
                    ...
                    print >>sys.stderr, "INFO: " + str(eid) + " Event found " + str(len(ev['events']))
                    assert len(ev['events']) > 0
            else:
                events.append(p['events'])
                print >>sys.stderr, "INFO: " +str(eid) + " Event found " + str(len(events[0]))
            ...
            if entity_type == 'TEZ_DAG_ID':
                ...
                expected_event_types = ['VERTEX_INITIALIZED', 'VERTEX_STARTED', 'VERTEX_PARALLELISM_UPDATED', 'VERTEX_FINISHED']
            elif entity_type == 'TEZ_TASK_ID':
            ...
            for evs in events:
                for i in evs:
                    print >>sys.stderr, "INFO: " + str(eid) + " EventType " + i['eventtype']
                    assert i['eventtype'] in expected_event_types is not None, \
                        "Got event %s where as expected is one of from %s in reponse %s%s" % (str(i['eventtype']),
                                                                                              ...
                                                                                              part_uri)
                    print >>sys.stderr, "INFO: " +str(eid) + " Event Type: " + i['eventtype'] + " time: " + str(i['timestamp'])
        ...
        if entity_type == 'TEZ_CONTAINER_ID':
            ...
            if 'endTime' in othic:
                ...
                print >>sys.stderr, "ERROR: In JSON Response for %s%s key='otherinfo' does not contains endTime" % (entity_type, part_uri)
            if 'exitStatus' in othic:
                assert re.match("^-?\d+$", str(othic['exitStatus'])) is not None, \
                 ...
                 print >>sys.stderr, "ERROR: In JSON Response for %s%s key='otherinfo' does not contains exitStatus" % (entity_type, part_uri)
            return True
        ...
        if entity_type in ['TEZ_DAG_ID', 'TEZ_VERTEX_ID', 'TEZ_TASK_ID', 'TEZ_TASK_ATTEMPT_ID']:
            if 'timeTaken' in othi:
                ...
                print >>sys.stderr, "ERROR: JSON Response for %s%s key='otherinfo' does not contains timeTaken" % (entity_type, part_uri)
            if 'endTime' in othi:
            ...
            if 'startTime' in othi:
                ...
                print >>sys.stderr, "ERROR: In JSON Response for %s%s key='otherinfo' does not contains startTime" % (entity_type, part_uri)
            if 'status' in othi:
                ...
                print >>sys.stderr, "ERROR: In JSON Response for %s%s key='otherinfo' does not contains status" % (entity_type, part_uri)
                        ...
                        for counter in counterGroup['counters']:
                            if 'counterName' in counter and 'counterValue' in counter:
                                ...
                                if counter['counterValue'] is not None and len(str(counter['counterValue'])) > 0:
                                    ...
                                    print >>sys.stderr, "ERROR: " + str(eid) + " Got counter value None or empty for counterName: " + \
                                                 str(counter['counterName']) + " value :" + str(counter['counterValue'])
                            else:
                                print >>sys.stderr, "ERROR: " + str(eid) + " Either counterName and counterValue is not there " + str(counter)
            ...
            if entity_type == 'TEZ_DAG_ID' or entity_type == 'TEZ_VERTEX_ID':
                if 'initTime' in othi:
                    ...
                    print >>sys.stderr, "ERROR: In JSON Response for %s%s key='otherinfo' does not contains initTime" % (entity_type, part_uri)
        ...
        if entity_type == 'TEZ_APPLICATION_ATTEMPT':
            if 'appSubmitTime' in othi:
                ...
                print >>sys.stderr, "ERROR: 'appSubmitTime' does exists in reponse of %s%s" % (entity_type, part_uri)
